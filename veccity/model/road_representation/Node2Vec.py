...
from veccity.model.abstract_replearning_model import AbstractReprLearningModel
...
class Node2Vec(AbstractReprLearningModel):
    ...
    def run(self, train_dataloader=None, eval_dataloader=None):
        ...
        json.dump(self.ind_to_geo, open('./veccity/cache/{}/evaluate_cache/ind_to_geo_{}.json'.format(
            self.exp_id, self.dataset), 'w'))
        json.dump(self.geo_to_ind, open('./veccity/cache/{}/evaluate_cache/geo_to_ind_{}.json'.format(
            self.exp_id, self.dataset), 'w'))
