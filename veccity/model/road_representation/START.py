...
class GATLayerImp3(GATLayer):
    ...
    def neighborhood_aware_softmax(self, scores_per_edge, trg_index, num_of_nodes):
        ...
        and it's a fairly common "trick" used in pretty much every deep learning veccity.
...
class GAT(nn.Module):
    ...
    def get_representation(self, node_features, edge_index_input, edge_prob_input):
        data = (node_features, edge_index_input, edge_prob_input)
        (node_fea_emb, edge_index, edge_prob) = self.gat_net(data)
        return node_fea_emb # vocab_size, d_model
...
class BERTEmbedding(nn.Module):
    ...
    def get_representation(self, graph_dict):
        return self.token_embedding.get_representation(node_features=graph_dict['node_features'],edge_index_input=graph_dict['edge_index'],edge_prob_input=graph_dict['loc_trans_prob'])
...
class BERT(nn.Module):
    ...
    def get_token_embedding(self,graph_dict):
        return self.embedding.get_representation(graph_dict)
