...
def get_executor(config, model, data_feature):
    ...
    """
    try:
        return getattr(importlib.import_module('veccity.executor'),
...
def get_model(config, data_feature):
    ...
    if config['task'] == 'traj_loc_pred':
        ...
        try:
            ...
            raise AttributeError('model is not found')
    elif config['task'] == 'line':
        try:
            return getattr(importlib.import_module('veccity.upstream.line'),
                           config['model'])(config, data_feature)
        except AttributeError:
            raise AttributeError('model is not found')
    elif config['task'] == 'region':
        try:
            return getattr(importlib.import_module('veccity.upstream.region'),
                           config['model'])(config, data_feature)
        except AttributeError:
            raise AttributeError('model is not found')
    elif config['task'] == 'point':
        try:
            return getattr(importlib.import_module('veccity.upstream.point'),
                           config['model'])(config, data_feature)
        except AttributeError:
            raise AttributeError('model is not found')
...
def need_train(config):
    ...
    embedding_path = f'veccity/cache/{exp_id}/evaluate_cache/{representation_object}_embedding_{model}_{dataset}_{output_dim}.npy'
