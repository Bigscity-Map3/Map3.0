...
class LINERegionDataset(TrafficRepresentationDataset):
    ...
    def __init__(self, config):
        ...
        self.cache_file_name = os.path.join('./libcity/cache/dataset_cache/',
                                            'region_rep_{}.npz'.format(self.parameters_str))
    ...
    def get_edges(self,adj_mx):
        """
        :param adj_mx: 邻接矩阵,np.ndarray
        :return: 所有的边集合，[[origin,destination,weight]....]，用于后续的负采样
        """
        self.edges = []
        for i in range(self.num_nodes):
            for j in range(self.num_nodes):
                if adj_mx[i][j]>0:
                    self.edges.append([i,j,adj_mx[i][j]])
        self.num_edges = len(self.edges)
        self._logger.info("finish construct graph,num_edges={}".format(self.num_edges))
