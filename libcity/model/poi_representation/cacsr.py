...
import itertools
from libcity.model.poi_representation.utils import construct_spatial_matrix_accordingDistance, tid_list_48,next_batch
...
class CacsrData:
    def __init__(self,config,data_feature):
        ...
        dirname = os.path.join(os.getcwd(), 'libcity','cache', 'dataset_cache', config['dataset'],'cacsr')
        if not os.path.exists(dirname):
            os.makedirs(dirname)
        train_save_filename = os.path.join(os.getcwd(), 'libcity','cache', 'dataset_cache', config['dataset'],'cacsr','train.npz')
        ss_save_filename = os.path.join(os.getcwd(), 'libcity','cache', 'dataset_cache', config['dataset'],'cacsr','ss.npz')
