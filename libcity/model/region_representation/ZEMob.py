...
class ZEMob(AbstractTraditionModel):
    ...
    def run(self, data=None):
        ...
        optimizer = optim.SGD(list(self.zones_embedding.parameters()) + list(self.events_embedding.parameters()), lr=0.001)
        batch_size=128
        ...
        for epoch in range(num_epochs):
            ...
            self._logger.info('Epoch [{}/{}], Loss: {:.4f}'.format(epoch + 1, num_epochs, total_loss))
        np.save(self.npy_cache_file, self.zones_embedding.weight.data.numpy())
        self._logger.info('词向量和模型保存完成')
        self._logger.info('词向量维度：' + self.zones_embedding.weight.shape)
