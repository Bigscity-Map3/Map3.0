...
class MVURE(AbstractTraditionModel):
    ...
    def run(self, data=None):
        ...
        time_step, item_num, _ = self.mob_adj.shape
...
class MVURE_Layer(nn.Module):
    def __init__(self,mob_adj,s_graph,t_graph,poi_graph,feature,input_dim,output_dim):
        ...
        self.s_gat = GATConv(in_channels=self.inputs.shape[-1],out_channels=self.output_dim,heads=12,dropout=0.2)
        self.t_gat = GATConv(in_channels=self.inputs.shape[-1],out_channels=self.output_dim,heads=12,dropout=0.2)
        self.poi_gat = GATConv(in_channels=self.inputs.shape[-1],out_channels=self.output_dim,heads=12,dropout=0.2)
    ...
    def caucalate_edge_index(self,adj_mx):
        """
        :param adj_mx:邻接矩阵，[num_nodes,num_nodes],np.array
        :return: edge_index,将邻接矩阵中大于0的全部算成一条边，Tensor([2*num_edges])
        """
        num_edges = np.count_nonzero(adj_mx)
        edge_index = torch.zeros([2,num_edges],dtype=torch.int)
    ...
    def forward(self):
        s_edge_index = self.caucalate_edge_index(self.s_graph)
        s_out = self.s_gat(self.inputs,s_edge_index)
        t_edge_index = self.caucalate_edge_index(self.t_graph)
        t_out = self.t_gat(self.inputs,t_edge_index)
        poi_edge_index = self.caucalate_edge_index(self.poi_graph)
        poi_out = self.poi_gat(self.inputs,poi_edge_index)
