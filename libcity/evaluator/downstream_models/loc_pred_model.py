...
def rnn_forward(encoder, sos, lstm_input, valid_len, pre_len):
    ...
    lstm_input = torch.cat([sos.reshape(1, 1, -1).repeat(batch_size, 1, 1), lstm_input],
                           dim=1)  # (batch, seq_len+1, 1+input_size)
    ...
    lstm_out_pre = torch.stack(
        [lstm_out[i, s - pre_len:s] for i, s in enumerate(valid_len)])  # (batch, pre_len, lstm_hidden_size)
    return lstm_out_pre
...
class StlstmLocPredictor(nn.Module, ABC):
    def __init__(self, embed_layer, num_slots, aux_embed_size, time_thres, dist_thres,
        ...
        self.encoder = nn.LSTM(input_size + 2 * aux_embed_size, lstm_hidden_size, num_layers, dropout=0.1,
                               batch_first=True)
        self.decoder = nn.LSTM(input_size + 2 * aux_embed_size, lstm_hidden_size, num_layers, dropout=0.1,
                               batch_first=True)
...
class STRNNCell(nn.Module):
    ...
    def forward(self, x_context, time_context, dist_context, context_mask, h):
        ...
        x_candidate = torch.matmul(x_context.unsqueeze(-2), torch.matmul(time_weight, dist_weight)).squeeze(
            -2)  # (batch, context_size, hidden_size)
...
class StrnnLocPredictor(nn.Module):
    ...
    def forward(self, full_seq, valid_len, pre_len, **kwargs):
        ...
        full_embed = self.embed_layer(full_seq, downstream=True, pre_len=pre_len,
                                      **kwargs)  # (batch, seq_len, embed_size)
        ...
        sequential_input = torch.stack([torch.cat([cat_input[i, :s], cat_input[i, -pre_len:],
                                                   torch.zeros(max_len - s, self.input_size + 3).float().to(
                                                       full_seq.device)], dim=0)
        ...
        context_mask = (time_delta <= self.time_window) * \
                       (time_delta >= 0) * \
                       (valid_len.unsqueeze(-1) > torch.arange(seq_len).to(full_seq.device).unsqueeze(0).repeat(
                           batch_size, 1)).unsqueeze(1)  # (batch, seq_len, seq_len)
        ...
        rnn_out, _ = self.encoder(sequential_input[:, :, :-3],
                                  ...
                                  context_mask)
        rnn_out_pre = torch.stack(
            [rnn_out[i, s - pre_len - 1:s - 1] for i, s in enumerate(valid_len)])  # (batch, pre_len, lstm_hidden_size)
...
class RnnLocPredictor(nn.Module, ABC):
    def __init__(self, embed_layer, input_size, rnn_hidden_size, fc_hidden_size, output_size, num_layers, seq2seq=True):
        ...
        self.encoder = nn.RNN(input_size, rnn_hidden_size, num_layers, dropout=0.1 if num_layers>1 else 0.0, batch_first=True)
        self.decoder = nn.RNN(input_size, rnn_hidden_size, num_layers, dropout=0.1 if num_layers>1 else 0.0, batch_first=True)
