...
def traj_user_classification(train_set, test_set, num_user, num_loc, clf_model, num_epoch, batch_size, task_lr, result_file_path,device):
    def _create_src_trg(origin, fill_value):
            src = np.transpose(np.array(list(zip_longest(*origin, fillvalue=fill_value))))
            return torch.from_numpy(src).float().to(device)

    logger = getLogger()
    logger.info('Start training downstream model [user_clf]...')
    clf_model = clf_model.to(device)
    optimizer = torch.optim.Adam(clf_model.parameters(), lr=task_lr)
    ...
    def one_step(batch):
        ...
        out = clf_model(inputs,length,user_index=user_index, timestamp=src_t,
                        ...
                        week=src_week,hour=src_hour,duration=src_duration,task="tul")
        return out, targets

    score_log = []
    test_point = max(1, int(len(train_set) / batch_size))
    ...
    for epoch in range(num_epoch):
        ...
        best_acc, best_pre, best_recall, best_f1_micro, best_f1_macro = np.max(score_log, axis=0)
                ...
                #     if patiences == 0:
                #         break 
        losses.append((total_loss/ (len(train_set) / batch_size)))
                
        logger.info('epoch {} complete!'.format(epoch))
        logger.info('mean loss: %.6f' % (total_loss/ (len(train_set) / batch_size)))
    ...
    with open(result_file_path,'wb') as f:
        ...
        logger.info("save tul result to {}".format(result_file_path))
...
class TrajectoryPredictor(nn.Module, ABC):
    def __init__(self, embed_layer, num_slots, aux_embed_size, time_thres, dist_thres,
        ...
        self.out_linear = nn.Sequential(nn.Tanh(), nn.Linear(lstm_hidden_size, fc_hidden_size),
                                        nn.Tanh(), nn.Linear(fc_hidden_size, output_size)) # for is static True
        # self.out_linear = nn.Sequential(nn.Tanh(), nn.Linear(input_size + 2 * aux_embed_size, fc_hidden_size),
        #                                 nn.Tanh(), nn.Linear(fc_hidden_size, output_size)) # for is static fasle
    ...
    def forward(self, full_seq, valid_len, pre_len, **kwargs):
        ...
        kwargs['valid_len']=valid_len
        # kwargs.pop("user_index")
        full_embed = self.embed_layer.encode(full_seq,**kwargs)  # (batch_size, seq_len, input_size)
        # full_embed = self.projector(full_embed)
    
        lstm_input = torch.cat([full_embed, aux_input],dim=-1)  # (batch_size, seq_len, input_size + aux_embed_size * 2)

        lstm_out_pre,out_len = seq2seq_forward(self.encoder, lstm_input, valid_len, pre_len) # for  is_static True

        lstm_out_pre=self.ln(lstm_out_pre) # for is static True
        out = self.out_linear(lstm_out_pre)
        return out


def loc_prediction(train_set, test_set, num_loc, pre_model, pre_len, num_epoch, batch_size,task_lr,result_file_path,device):
    def one_step(batch):
        def _create_src_trg(origin, fill_value):
        ...
        out = pre_model(src_seq, length, pre_len, user_index=user_index, timestamp=src_t,
                        time_delta=src_time_delta, dist=src_dist, lat=src_lat, lng=src_lng,
                        week=src_week,hour=src_hour,duration=src_duration,task="LP")
    ...
    for epoch in range(num_epoch):
        ...
        best_acc1, best_acc5, best_f1_micro, best_f1_macro = np.max(score_log, axis=0)
                ...
                #     if patiences == 0:
                #         break 
        mean_loss= total_loss /(len(train_set) / batch_size)    
        logger.info('epoch {} complete! avg loss:{}'.format(epoch,mean_loss))
    ...
    with open(result_file_path,'wb') as f:
        pickle.dump(losses,f)
        pickle.dump(score_log,f)
        logger.info("save lp result to {}".format(result_file_path))
